...
export function OnboardingPage(){
  ...
  async function submit(e:FormEvent){e.preventDefault();setBusy(true);const {error}=await supabase.rpc("create_financial_group",{group_name:name});if(error){setError(error.message);setBusy(false);return;}await refreshMembership();nav("/accounts");}
